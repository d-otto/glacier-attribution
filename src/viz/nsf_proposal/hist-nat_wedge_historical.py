...
ds_anth = d.sel(experiment=['historical'])
...
for key, dfg in df.groupby(['model']):
    ...
    if len(df_nat) > 0 and len(df_anth) > 0:
        ...
        ax.fill_between(ts, y1=sm_anth, y2=sm_nat, where=sm_anth > sm_nat, interpolate=True, fc='tab:red', ec='none', alpha=0.25)
        ax.fill_between(ts, y1=sm_anth, y2=sm_nat, where=sm_anth < sm_nat, interpolate=True, fc='tab:blue', ec='none', alpha=0.25)
...
plt.tight_layout()
fig.savefig(Path(ROOT, f'plots/nsf_proposal/hist-nat_wedge_historical_{rgiid}.png'))
fig.show()